...
void LookupTableFunction::setCalibDatas(float cam_in[9], float cam_dis[5], float pro_in[9], float pro_dis[5], float r[9], float t[3])
{
	cv::Mat camera_intrinsic = cv::Mat(3, 3, CV_32F, cam_in);
	cv::Mat camera_distortion = cv::Mat(1, 5, CV_32F, cam_dis);

	cv::Mat project_intrinsic = cv::Mat(3, 3, CV_32F, pro_in);
	cv::Mat projector_distortion = cv::Mat(1, 5, CV_32F,pro_dis);

	cv::Mat rotation_matrix = cv::Mat(3, 3, CV_32F, r);
	cv::Mat translation_matrix = cv::Mat(3, 1, CV_32F, t);
...
bool LookupTableFunction::generate_pointcloud(cv::Mat z, cv::Mat& mask, cv::Mat& map_xyz)
{
	...
	cv::Mat xyz_mat(img_height, img_width, CV_64FC3, cv::Scalar(0.0, 0.0, 0.0));
