...
/**@brief: 从相机获取备份标定文件
 * @param calibration_param :[out] :在相机内部保存的备份标定文件
*/
DF_SDK_API int DfGetCalibrationParamOriginal(struct CameraCalibParam& calibration_param);

/// @brief 输入标定文件以及查找表到相机内部
/// @param calibration_param [in] 标定文件结构体
/// @param rotate_x [in]
/// @param rotate_y [in]
/// @param rectify_r1 [in]
/// @param mapping [in]
/// @param mini_mapping [in]
/// @param width [in]
/// @param height [in]
/// @return 类型（int）:返回0表示获取数据成功;否则表示获取数据失败.
