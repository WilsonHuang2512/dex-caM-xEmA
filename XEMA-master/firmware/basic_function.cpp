...
int inquireVersion(std::string git_version, std::string& version)
{
	...
	{
		//1.5.5 版本之后改为 vx.x.x commit-id形式
		if(tokens.size()>=1)
		{
			if((*tokens.begin())[0]=='v')
			{
...
void initVersion()
{
	...
	map_version_.insert(std::pair<std::string, std::string>("2024-12-28", "v1.5.4"));
	map_version_.insert(std::pair<std::string, std::string>("2026-01-19", "v1.5.5"));
}
