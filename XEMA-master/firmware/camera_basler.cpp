...
CameraBasler::CameraBasler()
{
    ...
    pid = PIDController(Kp, Ki, Kd, target_brightness);
    min_exposure = 100;
    max_exposure = 500000; 
    min_gain=0;
    max_gain=23;
...
bool CameraBasler::grap(unsigned short* buf)
{
     std::lock_guard<std::mutex> my_guard(operate_mutex_);
        ...
        res = PylonWaitObjectWait( hWait_, 2000, &isReady );
        // CHECK( res );
        if (!isReady)
        {
            /* Timeout occurred. */
...
bool image_average_luminance(const cv::Mat& input_image, int roi_start_x, int roi_start_y, int roi_width, int roi_height, int& output_average_luminance)
{
    if (input_image.empty())
        return false;
    if (roi_start_x < 0 || roi_start_x + roi_width > input_image.cols || roi_start_y < 0 || roi_start_y + roi_height > input_image.rows || roi_width < 1 || roi_height < 1)
    {
        LOG_ERROR << "image_average_luminance";
        roi_start_x = 0;
        roi_start_y = 0;
        roi_width = input_image.cols;
        roi_height = input_image.rows;
        return false; 
    }
    ...
    {
        for (int row = roi_start_y; row < roi_start_y + roi_height; row += 1)
        {
            for (int col = roi_start_x; col < roi_start_x + roi_width; col += 1)
            {
                sum_luminance += input_image.at<unsigned char>(row, col);
            }
        }
    }
    else if (input_image.channels() == 3)
    {
        for (int row = roi_start_y; row < roi_start_y + roi_height; row += 1)
        {
            for (int col = 0; col < roi_start_x + roi_width; col += 1)
            {
                cv::Vec3b one_pixel = input_image.at<cv::Vec3b>(row, col);
                sum_luminance += one_pixel[0];
                sum_luminance += one_pixel[1];
                sum_luminance += one_pixel[2];
            }
        }
    }

    output_average_luminance = sum_luminance / (roi_width * roi_height * input_image.channels());

    return true;
}


bool planExposureAndGain(double input_exposure, double input_gain, double input_max_exposure, double input_max_gain, double& output_exposure, double& output_gain)
{

	if (input_exposure <= 0 || input_gain < 0 || input_max_exposure <= 0)
	{
		return false;
	}

	double equal_exposure = input_exposure * pow(10, (input_gain / 20.));
	if (equal_exposure > input_max_exposure)
	{
		double G_linear = equal_exposure / input_max_exposure;
		output_gain = 20. * std::log10(G_linear) > input_max_gain ? input_max_gain : 20. * std::log10(G_linear);
		output_exposure = input_max_exposure;
	}
	else
	{
		output_exposure = equal_exposure;
		output_gain = 0.;
	}
...
bool CameraBasler::capture_auto(unsigned char* buf,double max_exposure_scan,int target_brightness_scan)
{
 
    max_exposure=max_exposure_scan;
    target_brightness=target_brightness_scan;
    pid.setpoint = target_brightness;
    LOG(INFO) << "max_exposure: " << max_exposure;
    LOG(INFO) << "target_brightness: " << target_brightness;
    
    setExposure(current_exposure_value);
    ...
    cv::Mat output_img(image_height_, image_width_, CV_8U, buf);

    int luminance_now = 0;
    image_average_luminance(output_img, roi_start_x, roi_start_y, roi_width, roi_height, luminance_now);
    LOG(INFO) << "luminance_now: " << luminance_now;
    ...
    while (1)
    {
        LOG(INFO)<<"ExposureTime_before: "<<current_exposure_value;
        LOG(INFO)<<"Camera Gain_before: "<<current_gain;
        count_img_try += 1;
        if ( (count_img_try>11) || (luminance_now < target_brightness && current_exposure_value > max_exposure - 1&& current_gain >= max_gain - 0.1f) || (luminance_now > target_brightness && current_exposure_value < min_exposure + 1&&current_gain <= min_gain +0.1f))
        {
            LOG_ERROR << "bad tried times: " << count_img_try;
            // current_exposure_value=100000;
            // current_gain=0;
            pid.clear_memory(); 
            return true;
        ...
        double current_equal_exposure = current_exposure_value * std::pow(10.0f, current_gain / 20.0f);

        double adjustment = pid.update(luminance_now);
        LOG(INFO) << "adjustment: " << adjustment;
    
        double target_equal_exposure = current_equal_exposure * (1.0f + adjustment);
    
        double planned_exposure = 0.0f;
        double planned_gain = 0.0f;
        if (!planExposureAndGain(target_equal_exposure, 0.0f, max_exposure, max_gain, planned_exposure, planned_gain))
        {
            LOG_ERROR << "planExposureAndGain failed";
            return false;
        }

          if (planned_exposure < min_exposure)
          {
              planned_exposure = min_exposure;
              pid.clear_memory(); 
          }
  
          current_exposure_value = planned_exposure;
          current_gain = planned_gain;
          setExposure(current_exposure_value);
          setGain(current_gain);
          
          LOG(INFO)<<"Set Camera ExposureTime: "<<current_exposure_value;
          LOG(INFO)<<"Set Camera Gain: "<<current_gain;
          

        if (std::abs(luminance_now - target_brightness) < bright_tolerance)
        {
            LOG(INFO) << "good tried times: " << count_img_try;
            return true;
        }
        trigger_software();
        if (!grap(buf))
...
bool CameraBasler::openCamera()
{
    ...
    {
        LOG_ERROR << "No basler devices found.\n";
    ...
    char gen_api_error_buf[1024];
    //v1.5.5
    if (GENAPI_E_OK != res)
    {
        LOG_ERROR<<"pylon create device failed,error code:\t"<<res;
        size_t gen_api_error_buf_len = sizeof(gen_api_error_buf);
        GenApiGetLastErrorDetail(gen_api_error_buf, &gen_api_error_buf_len);
        LOG_ERROR<<"genapi:\t"<<gen_api_error_buf;
        return false;
    }
    ...
    if (GENAPI_E_OK != res)
    {
        LOG_ERROR<<"pylon open device failed,error code:\t"<<res;
        size_t gen_api_error_buf_len = sizeof(gen_api_error_buf);
        GenApiGetLastErrorDetail(gen_api_error_buf, &gen_api_error_buf_len);
        LOG_ERROR<<"genapi:\t"<<gen_api_error_buf;
        return false;
