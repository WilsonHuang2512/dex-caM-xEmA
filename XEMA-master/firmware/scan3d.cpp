...
int Scan3D::init()
{
    ...
    if (DF_SUCCESS != ret)
    {
        LOG_ERROR << "read patterns sets FAILED; CODE : " << ret;
    }
...
{
    LOG_ERROR<<"reopen Camera:";
    if(!camera_->closeCamera())
    {
        LOG_ERROR<<"close Camera Error!";
    }
    ...
    LOG_ERROR<<"reopen Camera finished!";
    return DF_SUCCESS;    
}
...
bool Scan3D::setParamLedCurrent(int current)
{
    ...
    {
        LOG_ERROR<<"Set Led Current failed";
        return false;
    }
...
bool Scan3D::setParamautoexposure(int model,float max_exposure,int target_brightness)
{
     if (model == 5)
    {
        generate_brightness_model_ = model;
        ...
        LOG(INFO) << "Set AutoExposure"<<max_exposure;
        LOG(INFO) << "Set Target_brightness_"<<target_brightness;
...
bool Scan3D::captureTextureImage(int model,float exposure,unsigned char* buff)
{
    ...
    case 5:
    {
        ...
        if(8 == bits)
        {
            if (!camera_->capture_auto(buff,double(max_exposure_),target_brightness_))
            {
                LOG(INFO) << "grap brightness failed!";
            }
            else
