...
#include <ctime>      // trigger lost error log
...
// triggerLostLog.txt 日志写入互斥锁
static std::mutex g_triggerLogMutex;
...
int reboot_lightcraft()
{
    ...
    while (operate_num-- > 0)
    {
        ...
        if (DF_SUCCESS != ret)
        {
            LOG_ERROR << "lc3010 init FAILED; CODE : " << ret;
            sleep(1);
        }
        else
        {
    ...
    while (operate_num-- > 0)
    {
        ...
        if (DF_SUCCESS != ret)
        {
            LOG_ERROR << "lc3010 SetLedCurrent FAILED; CODE : " << ret;
            usleep(100000);
        }
        else
        {
...
/**
 * @brief 异常重新采集
 * @param [in] captureFunc 采集函数
 * @return 采集函数状态码
 */
int recapture(const std::function<int()>& captureFunc)
{
    reboot_lightcraft();
    // 光机启动后3s内不能采集, 否则采集可能异常也可能正常
    std::this_thread::sleep_for(std::chrono::seconds(3));
    // reboot_lightcraft里已经设置了led_current, 但直接采集会出现深度图不一致情况
    // sleep 3s 后再次设置, 此时采集的深度图就一致了
    lc3010.SetLedCurrent(system_config_settings_machine_.Instance().config_param_.led_current,
                         system_config_settings_machine_.Instance().config_param_.led_current,
                         system_config_settings_machine_.Instance().config_param_.led_current);
    // 重新采集
    int ret = captureFunc();
 ...
 * @brief 错误码转字符串
 */
static const char* triggerErrorCodeToString(int code)
{
    if (code == DF_SUCCESS)
        return "DF_SUCCESS";
    else if (code == DF_ERROR_LOST_TRIGGER)
        return "DF_ERROR_LOST_TRIGGER";
    else if (code == DF_ERROR_CAMERA_GRAP)
...
/**
 * @brief 日志文件大小检查与轮转
 * @param [in] logFilePath 日志路径
 * @param [in] appendSize  追加日志的大小
 * @param [in] maxFileSize 文件最大存储空间限制
 */
static void rotateTriggerLogFileIfNeeded(const std::string& logFilePath, unsigned long long appendSize, unsigned long long maxFileSize)
{
    const unsigned long long currentSize = getFileSize(logFilePath);
    if (currentSize + appendSize <= maxFileSize)
    {
        return;
    }
    // 只保留一个备份文件, 如果 triggerLostLog.txt.old 已经存在先删除。
    const std::string backupFilePath = logFilePath + ".old";  
    std::remove(backupFilePath.c_str());
    // 当前日志重命名为 triggerLostLog.txt.old。
    if (std::rename(logFilePath.c_str(), backupFilePath.c_str()) != 0)
    {
        // 如果 rename 失败，则清空当前日志，防止日志无限增长。
...
/**
 * @brief 写入采集触发异常日志
 * @param [in] originalCode  触发异常的原始错误码
 * @param [in] recaptureCode 重新采集后的状态码
 */
void writeTriggerErrorLog(int originalCode, int recaptureCode)
{
    std::lock_guard<std::mutex> lock(g_triggerLogMutex);
    const std::string logFilePath = "triggerLostLog.txt";
    const unsigned long long MAX_TRIGGER_LOG_FILE_SIZE = 1ULL * 1024ULL * 1024ULL;
    auto timestamp = getCurrentTime();
    std::ostringstream oss;
    oss << timestamp << " [INFO] Capture ERROR." << std::endl;
    oss << timestamp << " [INFO] Original error code: " << originalCode << " -> "
        << triggerErrorCodeToString(originalCode) << std::endl;
    oss << timestamp << " [INFO] Recapture code: " << recaptureCode << " -> "
        << triggerErrorCodeToString(recaptureCode) << std::endl;
    const std::string logContent = oss.str();
    rotateTriggerLogFileIfNeeded(logFilePath, static_cast<unsigned long long>(logContent.size()), MAX_TRIGGER_LOG_FILE_SIZE);
 ...
 * @brief 异常处理
 * @param [in] code 错误码
 * @param [in] captureFunc 采集函数, 用于-15重新采集. 
 * @note captureFunc 默认为空, 为空时与之前保持逻辑一致.
 */
void handle_error(int& code, const std::function<int()>& captureFunc = {})
{
    if (DF_SUCCESS != code)
    {
        LOG_ERROR << "handle error: " << code;
        
        // [2026-04] NEW: 采集触发信号丢失, 重启光机再次采集
        if ((code == DF_ERROR_LOST_TRIGGER || code == DF_ERROR_CAMERA_GRAP) && captureFunc)
        {
            LOG(INFO) << "Find -13 or -15";
            LOG(INFO) << "The second collection begins";
            int errCode = code; // 仅用于日志输出
            code = recapture(captureFunc); // 更新采集状态码
            if (DF_SUCCESS != code)
            {
                LOG_ERROR << "Recapture failed. error code: " << code;             
            }
            writeTriggerErrorLog(errCode, code);
        }
...
int handle_cmd_connect(int client_sock)
{
    ...
    {
        LOG_ERROR << "get firmware version"; 
    }
...
int handle_cmd_get_focusing_image(int client_sock)
{
    ...
    if (ret == DF_FAILED)
    {
        LOG_ERROR << "send error, close this connection!";
        return DF_FAILED;
    }
...
int handle_cmd_get_frame_06_repetition_black(int client_sock)
{
    ...
    if (DF_SUCCESS != ret)
    {
    //   LOG_ERROR << "captureFrame04BaseConfidence code: " << ret;
    //   frame_status_ = ret;
...
int handle_cmd_get_frame_06_hdr_color(int client_sock)
{
    ...
    if(DF_SUCCESS != ret)
    { 
        //  LOG_ERROR<<"captureFrame06Hdr code: "<<ret;
        //  frame_status_ = ret;
...
int handle_cmd_get_frame_06_color(int client_sock)
{
    ...
    if(DF_SUCCESS != ret)
    { 
        LOG_ERROR<<"captureFrame06 code: "<<ret;
        //  frame_status_ = ret;
...
int handle_cmd_get_frame_04_parallel(int client_sock)
{
    ...
    { 
        ...
        //  LOG_ERROR<<"captureFrame04 code: "<<ret;
        //  frame_status_ = ret;
...
int read_calib_param()
{
    ...
    if(!ifile.is_open())
    {   
        LOG_ERROR<<"calib_param.txt not found!";
        ifile.open("calib_param_origin.txt");
        if(!ifile.is_open())
            return DF_FAILED;
    }
...
int read_calib_param_origin(CameraCalibParam& param_origin)
{
    std::ifstream ifile;
    ...
    int n_params = sizeof(CameraCalibParam)/sizeof(float);
    for(int i=0; i<n_params; i++)
    {
	ifile>>(((float*)(&param_origin))[i]);
    }
    ifile.close();
    return DF_SUCCESS;
}

int handle_get_camera_parameters_origin(int client_sock)
{
    if(check_token(client_sock) == DF_FAILED)
    {
	return DF_FAILED;
    }
    CameraCalibParam param_origin;
    read_calib_param_origin(param_origin);
	
    int ret = send_buffer(client_sock, (char*)(&param_origin), sizeof(param_origin));
    if(ret == DF_FAILED)
    {
        LOG(INFO)<<"send error, close this connection!\n";
	return DF_FAILED;
...
bool set_system_config(SystemConfigParam &rect_config_param)
{
    ...
    { 
        if(0<= rect_config_param.led_current && rect_config_param.led_current< 1024)
        {
            ...
            if(!scan3d_.setParamLedCurrent(brightness_current))
            {
                LOG_ERROR<<"Set Led Current: "<<brightness_current;
            }
            else
            {
                system_config_settings_machine_.Instance().config_param_.led_current = brightness_current;
...
int handle_cmd_set_param_auto_exposure(int client_sock)
{
 if(check_token(client_sock) == DF_FAILED)
    {
	    return DF_FAILED;
    ...
    float max_exposure_ser = 0;

    ret = recv_buffer(client_sock, (char*)(&max_exposure_ser), sizeof(float));
    if(ret == DF_FAILED)
    {
        LOG(INFO)<<"send error, close this connection!\n";
    	return DF_FAILED;
    ...
    int target_brightness_ser=0;
       ret = recv_buffer(client_sock, (char*)(&target_brightness_ser), sizeof(float));
    if(ret == DF_FAILED)
    {
        LOG(INFO)<<"send error, close this connection!\n";
    	return DF_FAILED;
    ...
    if (scan3d_.setParamautoexposure(flag, max_exposure_ser,target_brightness_ser))
    {
        generate_brightness_model = flag;
        //generate_brightness_exposure_time = exposure;
...
void copyFileWithOverwrite(const std::string& src, const std::string& dst) {
    // 以二进制模式打开源文件
    std::ifstream inFile(src, std::ios::binary);
    if (!inFile) {
        std::cerr << "无法打开源文件: " << src << std::endl;
        LOG_ERROR<<"calibrate file copy fail!";
        return;
    }

    // 以二进制模式打开目标文件
    // std::ios::trunc 表示如果文件存在，先截断（清空）内容，实现覆盖效果
    ...
    LOG(INFO)<<"calibrate file copy success!";
}

int handle_set_camera_looktable_origin(int client_sock)
{
    //先写入到 calib_param.txt
    int ret=handle_set_camera_looktable(client_sock);
    if(ret!=DF_SUCCESS)
    {
        return ret;
    }
    //再复制到 calib_param_origin.txt
    copyFileWithOverwrite("calib_param.txt","calib_param_origin.txt");
    return DF_SUCCESS;
}
...
int handle_get_mac(int client_sock)
{
    if(check_token(client_sock) == DF_FAILED)
    {
	    return DF_FAILED;
    ...
    {  
        LOG(INFO) << "Unable to open MAC address file, close this connection!\n";  
        return DF_FAILED;  
    }  
 
    char macStr[_VERSION_LENGTH_]; 
    std::strncpy(macStr, macAddress.c_str(), sizeof(macStr));  
    macStr[sizeof(macStr) - 1] = '\0';

    char version[_VERSION_LENGTH_] = _VERSION_NUMBER_;
    int ret = send_buffer(client_sock, macStr, _VERSION_LENGTH_);
    if(ret == DF_FAILED)
    {
        LOG(INFO)<<"send error, close this connection!\n";
	    return DF_FAILED;
...
void load_txt(std::string filename, char* info, int length)
{
	...
	if (fr != NULL) {
		__attribute__((unused)) size_t result = fread(info, 1, length, fr);
		fclose(fr);
	}
	else {
...
int handle_test_network_speed(int client_sock)
{
    if(check_token(client_sock) == DF_FAILED)
    {
	    return DF_FAILED;
    ...
    int ret = send_buffer(client_sock, buffer_temp, test_speed_size);
    LOG(INFO) << "speed test end!";
...
int handle_commands(int client_sock)
{
	...
	case DF_CMD_GET_CAMERA_PARAMETERS:
	    ...
	    break;
    case DF_CMD_GET_CALIB_ORIGIN:
	    LOG(INFO)<<"DF_CMD_GET_CALIB_ORIGIN";
	    ret = handle_get_camera_parameters_origin(client_sock);
	    break;
    case DF_CMD_SET_CALIB_ORIGIN:
	    LOG(INFO)<<"DF_CMD_SET_CALIB_ORIGIN";
	    ret = handle_set_camera_looktable_origin(client_sock);
	    break;    
    ...
    case DF_CMD_GET_MAC:
        LOG(INFO)<<"DF_CMD_GET_MAC";
        ret = handle_get_mac(client_sock);
        break;
	...
	case DF_CMD_SET_PARAM_GENERATE_BRIGHTNESS:
	    ...
	    break;
    case DF_CMD_SET_PARAM_AUTO_EXPOSURE:
	    LOG(INFO)<<"DF_CMD_SET_PARAM_AUTO_EXPOSURE";   
    	ret = handle_cmd_set_param_auto_exposure(client_sock);  
	    break;
    ...
    case DF_CMD_GET_PRODUCT_INFO:
        ...
        break;
    case DF_CMD_TEST_NETWORK_SPEED:
        LOG(INFO)<<"DF_CMD_TEST_NETWORK_SPEED"; 
        ret = handle_test_network_speed(client_sock);
        break;
...
{ 
    ...
    {
        __attribute__((unused)) size_t result = system("mkdir xemaLog");
    }
   ...
   sort(name_list.begin(), name_list.end());
	...
	for (int i = num; i > 0 && !name_list.empty(); i--)
	{
		...
		__attribute__((unused)) size_t result = system(ss.str().c_str());
	}
...
int main()
{
    ...
    //将error和fatal 的输出到额外日志文件
    // conf.set(el::Level::Error, el::ConfigurationType::Filename, "xema_error.log");
    // conf.set(el::Level::Fatal, el::ConfigurationType::Filename, "xema_error.log");
    // conf.set(el::Level::Warning, el::ConfigurationType::Filename, "xema_error.log");
    ...
    //v.1.5.5 error等级的日志 同时输出到xema_error.log
    el::Configurations ExtErrorLogConf;
	ExtErrorLogConf.setToDefault();
	ExtErrorLogConf.setGlobally(el::ConfigurationType::ToStandardOutput, "false");
	ExtErrorLogConf.setGlobally(el::ConfigurationType::ToFile, "false");
	ExtErrorLogConf.set(el::Level::Error, el::ConfigurationType::Format, "%datetime %level %msg");
	ExtErrorLogConf.set(el::Level::Error, el::ConfigurationType::ToFile, "true");
	ExtErrorLogConf.set(el::Level::Error, el::ConfigurationType::Filename, "xema_error.log"); 
	el::Loggers::reconfigureLogger(EXT_ERROR_LOGGER, ExtErrorLogConf);

	/// 注册回调函数
	el::Helpers::installPreRollOutCallback(rolloutHandler);

    /***************************************************************************/
    //v1.5.5 不管哪个文件都要输出启动日志
    LOG_ERROR<<"Just Notice:"<<"server started";
    ...
    //相机打开失败，闪烁6秒
        ...
        LOG_ERROR << "restart_camera_server!"; 
        //v1.5.5将系统重启改为进程重启
        //LOG_ERROR << "reboot!"; 
        //pwrctrl.off_board();
        return -1;
