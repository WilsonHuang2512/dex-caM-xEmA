...
int main()
{
	...
	ret_code = DfConnect("192.168.6.28");
	...
	if (0 == ret_code)
	{
		...
		// 测试相机网速
		double speed = 0;
		ret_code = DfTestNetworkSpeed(&speed);
		std::cout << "Speed Test Result: " << speed << "Mbps" << std::endl;

		// 打印产品信息
		std::string product_info;
		ret_code = DfGetProductInfo(&product_info);
		std::cout << "product_info: \n" << product_info << std::endl;
	}
	else
	{
	...
	//DfDisconnect(pBaseinfo[0].ip);
	DfDisconnect("192.168.6.28");
}
