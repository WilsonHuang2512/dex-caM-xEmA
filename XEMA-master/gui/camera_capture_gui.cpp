	...
	: QWidget(parent)
{
	...
	processing_gui_settings_data_.version = QString(version);
...
void CameraCaptureGui::setCalibrationBoard(int flag)
{
	...
	case 4:
	{
		...
		processing_gui_settings_data_.calibration_board = flag;
	}
	break;
...
bool CameraCaptureGui::loadSettingData(QString path)
{
	...
	processing_gui_settings_data_.ip = old_ip;
...
bool CameraCaptureGui::renderHeightImage(cv::Mat height)
{
	...
	float low_z = processing_gui_settings_data_.low_z_value;
	float high_z = processing_gui_settings_data_.high_z_value;
...
bool CameraCaptureGui::setShowImages(cv::Mat brightness, cv::Mat depth)
{
	...
	int low_z = processing_gui_settings_data_.low_z_value;
	int high_z = processing_gui_settings_data_.high_z_value;
...
{  
	...
	do_comboBox_activated_engine(processing_gui_settings_data_.engine);
...
{
	ui.doubleSpinBox_min_z->setValue(processing_gui_settings_data_.low_z_value);
	ui.doubleSpinBox_max_z->setValue(processing_gui_settings_data_.high_z_value);
	ui.lineEdit_ip->setText(processing_gui_settings_data_.ip);
	ui.spinBox_repetition_count->setValue(processing_gui_settings_data_.repetition_count);
	
	//ui.spinBox_camera_exposure->setValue(firmware_config_param_.camera_)

	setCalibrationBoard(processing_gui_settings_data_.calibration_board);
	qDebug() << "processing_gui_settings_data_.calibration_board: " << processing_gui_settings_data_.calibration_board;
	//ui.spinBox_exposure_num->setDisabled(true);
	//ui.spinBox_led->setDisabled(true);

	switch (processing_gui_settings_data_.exposure_model)
	{
	case 0:
	{
	...
	if (processing_gui_settings_data_.show_over_exposure)
	{
	...
	if (processing_gui_settings_data_.auto_save)
	{
	...
	switch (processing_gui_settings_data_.show_image_flag)
	{
	...
	radio_button_flag_ = processing_gui_settings_data_.show_image_flag;
	showImage();

	switch (processing_gui_settings_data_.engine)
	{
...
{
	processing_gui_settings_data_.low_z_value = val;
...
{
	processing_gui_settings_data_.repetition_count = val;
}
...
void CameraCaptureGui::do_spin_max_z_changed(double val)
{
	...
	processing_gui_settings_data_.high_z_value = val;
...
void CameraCaptureGui::captureOneFrameBaseThread(bool hdr)
{
	...
	{
		//重复曝光
		//ret_code = DfGetRepetitionFrame04(processing_gui_settings_data_.repetition_count, (float*)depth.data, depth_buf_size, (uchar*)brightness.data, brightness_bug_size);
		exposure_num = processing_gui_settings_data_.repetition_count;
...
bool CameraCaptureGui::captureOneFrameData()
{
	...
	if (0 == ret_code)
	{
		...
		processing_gui_settings_data_.exposure_model = exposure_model_;
...
void CameraCaptureGui::do_pushButton_refresh()
{
	...
	//更新相机设备列表
	int camera_num = 0;
	ret_code = DfUpdateDeviceList(0, camera_num);
	...
	do {
		...
		if (!connected_flag_)
		{
			...
			if (DF_SUCCESS == ret_code)
			{
				...
				//保存ip配置
				processing_gui_settings_data_.ip = camera_ip_;
...
void CameraCaptureGui::do_pushButton_save_as()
{
	...
	processing_gui_settings_data_.last_path = last_path_; 
...
void CameraCaptureGui::do_comboBox_activated_engine(int index)
{
	...
	case 0:
	{
		...
		processing_gui_settings_data_.engine = index;
	}
	break;
	...
	qDebug() << "dd engine: " << processing_gui_settings_data_.engine;
}
...
void CameraCaptureGui::do_checkBox_toggled_over_exposure(bool state)
{
	...
	showImage();
	processing_gui_settings_data_.show_over_exposure = state;
}
...
{
	processing_gui_settings_data_.auto_save = state;
  
}

void CameraCaptureGui::do_checkBox_toggled_hdr(bool state)
{
	processing_gui_settings_data_.use_hdr_model = state;
	if (connected_flag_)
	{
		if (state)
		{
...
void CameraCaptureGui::do_QRadioButton_toggled_brightness(bool state)
{
	...
	{
		radio_button_flag_ = SELECT_BRIGHTNESS_FLAG_;
		processing_gui_settings_data_.show_image_flag = SELECT_BRIGHTNESS_FLAG_;
...
void CameraCaptureGui::do_QRadioButton_toggled_color_depth(bool state)
{
	if (state)
	{
		...
		processing_gui_settings_data_.show_image_flag = SELECT_COLOR_DEPTH_FLAG_;
		//qDebug() << "state: " << radio_button_flag_;
		showImage();
	}
}
...
void CameraCaptureGui::do_QRadioButton_toggled_color_height(bool state)
{
	if (state)
	{
		...
		processing_gui_settings_data_.show_image_flag = SELECT_HEIGHT_MAP_FLAG_;
		//qDebug() << "state: " << radio_button_flag_;
		showImage();
	}
}
...
void CameraCaptureGui::updateLanguage()
{
	...
	ui.comboBox_engine->setCurrentIndex(processing_gui_settings_data_.engine);
}
