...
\n\
37.Enable Checking Sharpness: \n\
open_cam3d.exe --enable-checking-sharpness --ip 192.168.x.x \n\
\n\
37.Set Back-Up Calibration File To Camera: \n\
open_cam3d.exe --set-origin-calib-file --ip 192.168.x.x --path calib_params_origin.txt\n\
\n\
38.Get Back-Up Calibration File From Camera: \n\
open_cam3d.exe --get-origin-calib-file --ip 192.168.x.x --path calib_params_origin.txt\n\
\n\
";
/**
37.Set Bakc-Up Calibration File To Camera: \n\
open_cam3d.exe --set-origin-calib-file --ip 192.168.x.x --path calib_params_origin.txt\n\
\n\
*/
...
int get_calib_param(const char* ip, const char* calib_param_path,bool from_original=false);
...
int set_calib_looktable(const char* ip, const char* calib_param_path,bool to_original=false);
...
enum opt_set
{
	...
	SET_BOARD_INSPECT,
	GET_PRODUCT_INFO,
	GET_ORIGIN_CALIB_FILE,
	//SET_ORIGIN_CALIB_FILE,
};
...
static struct option long_options[] =
{
	...
	//{"set-origin-calib-file",no_argument,NULL,SET_ORIGIN_CALIB_FILE},
	{"get-origin-calib-file",no_argument,NULL,GET_ORIGIN_CALIB_FILE},
};
...
int main(int argc, char* argv[])
{
	...
	case SET_CALIB_LOOKTABLE:
		set_calib_looktable(camera_id, path, true);
		break;
	...
	case GET_PRODUCT_INFO:
		...
		break;
	//case SET_ORIGIN_CALIB_FILE:
	//	set_calib_looktable(camera_id,path,true);
	//	break;
	case GET_ORIGIN_CALIB_FILE:
		get_calib_param(camera_id,path,true);
		break;
	default:
		break;
	}
...
int get_calib_param(const char* ip, const char* calib_param_path,bool from_original)
{
	DfRegisterOnDropped(on_dropped);
	...
	{
		DfGetCalibrationParamOriginal(calibration_param);
	}
	std::ofstream ofile;
...
int set_calib_looktable(const char* ip, const char* calib_param_path,bool to_original)
{
	/*************************************************************************************************/
	...
	{
		DfSetCalibrationLookTableOriginal(calibration_param, (float*)xL_rotate_x.data, (float*)xL_rotate_y.data, (float*)rectify_R1.data,
		(float*)pattern_mapping.data, (float*)pattern_minimapping.data, width, height);
	}
...
int configure_focusing(const char* ip)
{
	...
	while (num-- > 0)
	{
		...
		cv::Point textOrg((img.cols - textSize.width) / 2, (img.rows + textSize.height) / 2 - 400);
...
int configure_dlp_checking(const char* ip)
{
	...
	ret = DfSetParamCameraExposure(20000);
	//cv::waitKey(10);
	if (DF_SUCCESS != ret)
	{
		std::cout << "Set Led Failed! " << std::endl;
	...
	while (num-- > 0)
	{
		...
		// 绝对清晰度
		SharpnessResult sr;
		sr = computeProjectorSharpnessByChessboard(img, 14, 11, 14, 15, good_size);
		...
		{
			cv::imwrite("checkboard.bmp", img_ori);

			std::cout << "checkboard.bmp saved!" << std::endl;
		}
