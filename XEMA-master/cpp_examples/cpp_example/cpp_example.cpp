...
int main()
{
	...
	ret_code = p_camera->connect("192.168.6.28");
	...
	if (0 == ret_code)
	{
		...
		ret_code = p_camera->testNetworkSpeed(&network_speed);
		std::cout << "Speed Test Result: " << network_speed << "Mbps" << std::endl;

		// 打印产品信息
		std::string product_info;
		ret_code = p_camera->getProductInfo(&product_info);
	...
	p_camera->disconnect("192.168.6.28");
