...
// Continuously measures 3D calibration board precision every 20 minutes.
// - Logs every result to precision_log_<date>.csv
// - Generates precision_chart_<date>.html (interactive chart) every 6 hours and on Ctrl+C / exit
//
// Usage:
...
int main(int argc, char* argv[])
{
    ...
    auto next_capture = steady_clock::now();           // capture immediately on start
    ...
    while (g_running) {
        ...
        if (now >= next_capture) {
            ...
            writeCSV(csv_path, records);               // append-safe: rewrite each time
            ...
            std::cout << "  next capture at " << buf << "\n";
        }

        // Time for a chart export?
        if (duration_cast<hours>(now - last_chart) >= chart_every) {
